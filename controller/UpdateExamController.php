...
class UpdateExamController
{
    ...
    public function isExamInResult($examId, $studentId){
        $ResultController = new GetResultController();

        $result = $ResultController->getResult($examId, $studentId);
    ...
    public function getCompletedExams($studentId)
    {
        ...
        return array_filter($exams, function ($exam) use ($now) {
            return $exam->end_time->toDateTime() < $now;
        });
    }
    ...
    public function getUpcomingExams($studentId)
    {
        ...
        return array_filter($exams, function ($exam) use ($now) {
            return $exam->start_time->toDateTime() > $now;
        });
    }
